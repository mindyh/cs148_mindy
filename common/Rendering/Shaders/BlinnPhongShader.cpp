...
std::array<const char*, 5> BlinnPhongShader::MATERIAL_PROPERTY_NAMES = {
...
void BlinnPhongShader::SetupShaderLighting(const Light* light) const
{
    if (!light) {
        ...
        SetShaderUniform("genericLight.light_radius", lightProperty->light_radius);
...
void BlinnPhongShader::UpdateMaterialBlock() const
{
    ...
    memcpy((void*)(materialStorage.data() + materialOffsets[4]), &is_affected_by_light_, sizeof(bool));
...
void BlinnPhongShader::SetLight(bool inLight) 
{ 
